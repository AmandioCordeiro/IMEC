...
*/
	...
	control_loops::control_loops():c(0),imrref(0.0),angle(0.0),abc_current(0.0,0.0,0.0),abc_voltage(0.0,0.0,0.0),IDQ(0.0001,0.0001),VDQ(0.0,0.0),rot_fl_an(0.0),Theta_r(0.0),w_ref(0.0),V(0.0,0.0){
			vel.tune_pid(vel_p,vel_i,vel_d);
...
//----------------------------------------
			abc_current.a=motor::get_ias();cout<<"ia"<<abc_current.a<<endl;abc_current.b=motor::get_ibs();abc_current.c=-abc_current.a-abc_current.b;//TODO: for real motor
	...
	if(n>5000){
		if(c==0){
			velo=motor::wr;Rs_Tr* ConstTr;ConstTr=new Rs_Tr();ConstTr_g=ConstTr;/*delete ConstTr;ConstTr=nullptr;*/}
		if (c<100){
			c++;
			ConstTr_g->enter_medition(IDQ_rotor.d/*f.filtred_isx()*/,IDQ_rotor.q/*f.filtred_isy()*/,VDQ_rotor.d/*f.filtred_usx()*/,VDQ_rotor.q/*f.filtred_usy()*/,motor::wr/*f.filtred_()*/);
		};
		if(c==100/*&&((1.10*velo)>(motor::wr)&&(velo*0.9)<(motor::wr))*/){
			ConstTr_g->Rs_Tr_do_it();
			c=0;
			motor::TR=ConstTr_g->get_Tr();/*motor::Rs=ConstTr->get_Rs();*/cout<<"TR::::"<<motor::TR<<endl/*<<"Rs:"<<motor::Rs<<endl*/;
			//Rs_Tr* ConstTr;ConstTr_g=ConstTr;
		};
	};//TODO
