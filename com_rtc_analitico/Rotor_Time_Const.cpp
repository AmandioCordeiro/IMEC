...
double J=(0.089*3);//TODO:?? 1pv5135-4ws14: 0.071
double ro=(1-(M*M)/(Ls*Lr));//0.999863//1-M*M/(Ls*Lr)//alterar nome

double VDC=300;//220;//312.0;//need actualization in pratice
...
//public:
	Y_W_::Y_W_():Rss/*0.258*/(0.012/*0.10941*/),//*0.435*/),//oooo
 Trr(/*1.12252*//*TODO???m_*/Tr_calc_gl/*0.17703*/),/*Lr/Rrr*/////ooo
...
float Rs_Tr::do_it(){
	...
	if (k2.size()>0){//15_07for(int i=0;i<k2.size();i++){cout<<"kkkkkkkkkkkkkkkkkkkkk222222222222222:"<<k2[i]<<endl;};cout<<"iiiiiii"<<endl;
		...
		int i_ = k2.size()-1;
		while(i_>=0){
			double g=-fx(a0,7,k2[i_])/fx(a1,6,k2[i_]);//TODO!!!???
			if (g>0) {
			  k1.insert(k1.begin(),g);}
			else{
			  k2.erase(k2.begin()+i_);}
			i_--;
		}/*k1.push_back(-fx(a0,7,k2[i])/fx(a1,6,k2[i]));*/ //15_07 for(unsigned i=0;i<k1.size();i++){cout<<"kkkkkkkkkkkkkkkkkkkkk1111111111111111111:"<<k1[i]<<endl;};cout<<"iiiiiii"<<endl;
ofstream Tr_sk("Tr.txt",ios::app);
Tr_sk<<"k1 size "<<k1.size()<<"k2 size "<<k2.size()<<endl;
Tr_sk.close();
if (k1.size()>0){
		...
		//for(unsigned i=0;i<k2.size();++i)//estva errado, nao é combinacao mas sim par de k2
			...
			if(min_Ep2__k1_j__k2_i==0.0)
			  {min_Ep2__k1_j__k2_i=temp;k1jf=j;k2if=i;}
			else{ 
			  if(temp<min_Ep2__k1_j__k2_i)
			  {min_Ep2__k1_j__k2_i=temp;k1jf=j;k2if=i;}
				}
			};
	...
	;
		...
		/*hessian.open("hessian.txt");*/ofstream hessian("hessian.txt",ios::app);
...
void Rs_Tr::Rs_Tr_do_it(){
	...
	if (Y_W_::n==3){
		...
		if (/*((h00*h11-h01*h10)>0) && (!isinf(h00*h11-h01*h10)) &&*/ /*zeros_pol==true*/E_2!=0)//TODO
		{	
			
			//cout<<"nada"<<endl;
			ofstream Tr_sk("Tr.txt",ios::app);
			Tr_sk<<" k2size "<<k2.size()<<" k1size "<<k1.size()<< endl;
			Tr_sk<<"k2if:"<<k2if<<endl/*<<"true  "*/<<"k2[k2if]: "<<k2[k2if]<<endl;
			for (int i=0;i<k2.size();i++){Tr_sk<<"k2 vector indice "<<i<<" :"<<k2[i]<<endl;};
			for (int i=0;i<k1.size();i++){Tr_sk<<"k1 vector indice "<<i<<" :"<<k1[i]<<endl;};
			Tr_sk.close();
			...
			//pei->get_dek2_pol();//atencao n colocar antes devido a flag zeros_pol, existem 2 uma rtc e outra pei...
			if( ( (1/k2[k2if]) < 1.5*Trrr) && ((1/k2[k2if]) > 0.5*Trrr) /*&& (pei->get_dek2_pol() < 2.3)*/ ){//1.6 pei
				Trr=(1/k2[k2if]);Tr_calc_gl=Trr;}
			else Trr=Tr_calc_gl;//TODO julgo n ser necessario isto pois no contrutor e atribuido o valor d Tr_calc_gl
			/*ofstream*/ Tr_sk.open("Tr.txt",ios::app);
			Tr_sk<<"1/k2[k2if]: "<<1/k2[k2if]<<endl;
			Tr_sk.close();
