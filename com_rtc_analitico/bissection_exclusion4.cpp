...
//extern std::atomic<bool> fim_ciclo;
//****************
//#define get_rk2 a0a1b1
bool zeros_pol=false;//TODO atencao 2 variaveis pois PEI tb utiliza isto
...
double exclusion(double *rk2, int dim_rk2, complex<double> x, double s){//true if m>0
	...
	for (int i=1;i<=dim_rk2/*num zeros*/;i++){
		...
		dim_der--;
		M_=M_+(abs(fx(derivada, dim_der, x))*pow((s*1.41421356237),i)/factorial(i));
		
	};
return (abs(fx(rk2_c, dim_rk2, x))-M_);
};
...
//	for(int e=0;e<=dim_rk2;e++)rk2_c[e]=get_rk2[e]/ladoo;////
		...
		for(int i=0;i<dim_rk2;i++){
			//TODOwhile(!fim_ciclo)std::this_thread::yield();
			get_rk2[i]=(get_rk2[i]/get_rk2[dim_rk2]);};get_rk2[dim_rk2]=1;//for(int i=0;i<=dim_rk2;i++)cout<<get_rk2[i]<<endl;

double lado=find_side_s0(get_rk2,dim_rk2);
lado=80000;//990
ofstream Ter("Ter.txt",ios::app);Ter<<" lado: "<<lado<<endl;Ter.close();
//lado=990;//TODO:Qual tamanho lado?nao e necessaria funcao anterior, limites escolhidos a priori
//cout<<"lllllllllllllllllllllllllllaaaaaaaaaaaaaddddddddddoooooooo"<<lado<<endl;//TODO qual o lado?
...
forward_list<pair<complex<double>,double> > z=find_squares(poli,tam_poli);
	vector<double> f;
	zeros_pol=false;//TODO julgo n ser necessario esta flag, n tenho certeza
	for(auto it=z.begin();it!=z.end();++it){
		if(abs(get<0>(*it).imag())<valImagMin && get<0>(*it).real()>0) {f.push_back(get<0>(*it).real());zeros_pol=true;};
						...
						//		for(vector<float>::iterator i=it;i!=f.end();++i){ 
							...
							//		}
	if(zeros_pol==true){						//	}
	for(unsigned  it=0;it<(f.size()-1);++it){//TODO:acho q isto é para eliminar repetidos
