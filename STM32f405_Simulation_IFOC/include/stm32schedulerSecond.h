 ...
 */
#ifndef STM32SCHEDULERSECOND_H
#define STM32SCHEDULERSECOND_H
#include <stdint.h>
...
class Stm32SchedulerSecond
{
   public:
       ...
       */
      Stm32SchedulerSecond(uint32_t timer);
...
int GetCpuLoad2();
   ...
   private:
      ...
      static const enum tim_oc_id ocMap2[MAX_TASKS];
      //void (*functions[MAX_TASKS]) (void);
      //uint16_t periods[MAX_TASKS];
      //uint16_t execTicks[MAX_TASKS];
      void (*functions2[MAX_TASKS]) (void);
      uint16_t periods2[MAX_TASKS];
      uint16_t execTicks2[MAX_TASKS];
      uint32_t timer;
      //int nextTask;
      int nextTask2;
};

#endif // STM32SCHEDULERSECOND_H
