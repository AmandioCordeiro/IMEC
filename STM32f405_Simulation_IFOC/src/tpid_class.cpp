...
_pid::_pid()
{
 ...
 this->pid_result_ = FP_FROMFLT( 10 );			  
 this->last_pid_result_ = FP_FROMFLT( 10 ) ;	
};
...
void _pid::init_pid(/*int*/s32fp pp,/*int*/s32fp sp, s32fp min, s32fp max, s32fp accel)
{
	...
	this->pid_result_ = FP_FROMFLT( 10 );//pp;
	this->setpoint_ = sp;
...
s32fp _pid::calc_pid()
{
  ...
  // Derivitive
  s32fp d;
	// only 3 set points, this, next, setpoint. last_setpoint not used
  ...
  if (this->acceleration_limit_ > 0 && this->/**this**/next_setpoint_ != this->setpoint_){
    // If we're below the user sp
    if (this->/**this**/next_setpoint_ < this->setpoint_){
  ...
  this->current_err_ = this->/**this**/next_setpoint_ - this->process_point_;//TODO?? not next?
...
//  d = this->current_err_ - this->last_err_;
  ...
  //};
  s32fp ppp = FP_MUL(this->pg_ , this->current_err_);
  
	if ( ( ppp + FP_MUL(this->ig_ , ( this->i_ + current_err_ ) ) ) > this->minimum_pid_ && ( ppp + FP_MUL(this->ig_ , ( this->i_ + current_err_ ) ) ) < this->maximum_pid_ ){//TODO
		this->i_ += current_err_;// this->next_setpoint_ - this->process_point_;
	}
	else
	{
		this->i_ -= current_err_;
	};
  ...
  s32fp pid_result = ( ppp + FP_MUL(this->ig_ , this->i_)/**+ FP_MUL(this->dg_ , d**/);//TODO remove
if ( pid_result > this->minimum_pid_ && pid_result < this->maximum_pid_ )
  {//TODO 
	  this->pid_result_ = pid_result;
}
//else 
//	pid_result_ = last_pid_result_ ;
...
////  this->last_err_ = this->current_err_;
 ...
 //TODO can remove?? 
 if (/**this->**/pid_result/**_**/ < this->minimum_pid_) {
		this->pid_result_ = this->minimum_pid_;
		/*this->i_ = this->minimum_pid_;*/ }
  else if (/**this->**/pid_result/**_**/ > this->maximum_pid_){
