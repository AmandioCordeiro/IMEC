...
void Encoder::InitTimerSingleChannelMode()
{
   ...
   gpio_mode_setup(NORTH_EXC_PORT, GPIO_MODE_INPUT, GPIO_PUPD_NONE, NORTH_EXC_PIN);
   ...
   if (hwRev == HW_REV1)
	dma_disable_stream (DMA2, REV_CNT_DMASTREAM);
   else
    dma_disable_stream(DMA1, REV_CNT_DMASTREAM);
   
   
   //TODO STM32F4 AMRC ??
   //dma_set_peripheral_address(DMA1, REV_CNT_DMACHAN, REV_CNT_CCR_PTR);
   if (hwRev == HW_REV1)
	dma_set_peripheral_address(DMA2, REV_CNT_DMASTREAM, REV_CNT_CCR_PTR);
   else
   dma_set_peripheral_address(DMA1, REV_CNT_DMASTREAM, REV_CNT_CCR_PTR);
   
   //TODO STM32F4 AMRC ??
   //dma_set_memory_address(DMA1, REV_CNT_DMACHAN, (uint32_t)timdata);
   if (hwRev == HW_REV1)
	dma_set_memory_address(DMA2, REV_CNT_DMASTREAM, (uint32_t)timdata);
   else
   dma_set_memory_address(DMA1, REV_CNT_DMASTREAM, (uint32_t)timdata);
   
   
   //TODO STM32F4 AMRC ??
   //dma_set_peripheral_size(DMA1, REV_CNT_DMACHAN, DMA_CCR_PSIZE_16BIT);
   if (hwRev == HW_REV1)
	dma_set_peripheral_size(DMA2, REV_CNT_DMASTREAM, DMA_SxCR_PSIZE_16BIT);//??
   else
   dma_set_peripheral_size(DMA1, REV_CNT_DMASTREAM, DMA_SxCR_PSIZE_16BIT);//??
   
   //TODO STM32F4 AMRC ??
   //dma_set_memory_size(DMA1, REV_CNT_DMACHAN, DMA_CCR_MSIZE_16BIT);
   if (hwRev == HW_REV1)
	dma_set_memory_size(DMA2, REV_CNT_DMASTREAM, DMA_SxCR_PSIZE_16BIT);//??
   else
   dma_set_memory_size(DMA1, REV_CNT_DMASTREAM, DMA_SxCR_PSIZE_16BIT);//??
   
   
   //TODO STM32F4 AMRC ??
   //dma_set_number_of_data(DMA1, REV_CNT_DMACHAN, MAX_REVCNT_VALUES);
   if (hwRev == HW_REV1)
	dma_set_number_of_data(DMA2, REV_CNT_DMASTREAM, MAX_REVCNT_VALUES);//??
   else
   dma_set_number_of_data(DMA1, REV_CNT_DMASTREAM, MAX_REVCNT_VALUES);//??
   
   //TODO STM32F4 AMRC ??
   //dma_enable_memory_increment_mode(DMA1, REV_CNT_DMACHAN);
   if (hwRev == HW_REV1)
	dma_enable_memory_increment_mode(DMA2, REV_CNT_DMASTREAM);
   else
	dma_enable_memory_increment_mode(DMA1, REV_CNT_DMASTREAM);
   
   //TODO STM32F4 AMRC ??
  // dma_enable_circular_mode(DMA1, REV_CNT_DMACHAN);
   if (hwRev == HW_REV1)
	dma_enable_circular_mode(DMA2, REV_CNT_DMASTREAM);
   else
	dma_enable_circular_mode(DMA1, REV_CNT_DMASTREAM);
   
   //TODO STM32F4 AMRC ??
   //dma_enable_channel(DMA1, REV_CNT_DMACHAN);
	if (hwRev == HW_REV1 ){
		dma_enable_stream(DMA2, DMA_STREAM0);
		dma_channel_select(DMA2, DMA_STREAM0, 1);//??DMA_SxCR_CHSEL_4);
	}
	else {
		dma_enable_stream(DMA1, DMA_STREAM5);
		dma_channel_select(DMA1, DMA_STREAM5, 1);//??DMA_SxCR_CHSEL_4);
	}
}
...
void Encoder::InitSPIMode()
{
   ...
   gpio_mode_setup(NORTH_EXC_PORT, GPIO_MODE_OUTPUT, GPIO_PUPD_NONE, NORTH_EXC_PIN);
...
void Encoder::InitResolverMode()
{
   ...
   adc_enable_external_trigger_injected(ADC1,  ADC_CR2_JEXTSEL_MASK, ADC_CR2_JEXTEN_DISABLED  ); //TODO STM32F4 AMRC?? ADC_CR2_JEXTSEL_MASK?? ADC_CR2_JEXTEN_DISABLED ??ADC_CR2_JEXTSEL_JSWSTART);
   adc_set_sample_time(ADC1, 6, ADC_SMPR_SMP_3CYC);//TODO STM32F4 AMRC ADC_SMPR_SMP_1DOT5CYC);
   adc_set_sample_time(ADC1, 7, ADC_SMPR_SMP_3CYC);//TODO STM32F4 AMRC ADC_SMPR_SMP_1DOT5CYC);
//TODO STM32F4 AMRC
   //gpio_set_mode(GPIOA, GPIO_MODE_INPUT, GPIO_CNF_INPUT_ANALOG, GPIO6 | GPIO7);
   gpio_mode_setup(GPIOA, GPIO_MODE_ANALOG, GPIO_PUPD_NONE, GPIO6 | GPIO7);//??GPIO_MODE_INPUT
   ...
   if (encMode == RESOLVER)
   {
      ...
      adc_enable_external_trigger_injected(ADC1, ADC_CR2_JEXTSEL_TIM3_CC4, ADC_CR2_JEXTEN_DISABLED);
...
uint16_t Encoder::GetAngleSPI()
{
   ...
   //TODO STM32F4 AMRC
   gpio_clear(NORTH_EXC_PORT, NORTH_EXC_PIN);//???GPIO_BRR(NORTH_EXC_PORT) = NORTH_EXC_PIN;

   for (int i = 15; i >= 0; i--)
   {//TODO STM32F4 AMRC
     gpio_clear(GPIOA,GPIO7);//?? GPIO_BRR(GPIOA) = GPIO7;
...
int Encoder::GetPulseTimeFiltered()
{
  ...
  //int n = dma_get_number_of_data(DMA1, REV_CNT_DMACHAN);
   int n;
   if (hwRev == HW_REV1)
	 n = dma_get_number_of_data(DMA2, REV_CNT_DMASTREAM);
   else
	 n = dma_get_number_of_data(DMA1, REV_CNT_DMASTREAM);
