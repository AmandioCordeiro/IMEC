...
RotFluxAng::RotFluxAng(/*float imr_*/)//TODO necess rfa?? para invocar construc? NEW
{
	...
	theta_r=0.00001;
	theta_slip=0.00001;
	imr = 0.00001;//TODO(remove)nao sera necessario se nao voltage compensator
	wm = FLT_FROMFP(1);
...
static float Rfe_ = 0 ;
static float Tfe = 0 ;
static float T_ro_r = 0 ;
static float F0 = 0 , idm0 = 0, iqm0 = 0;
float RotFluxAng::/*CalcSlipAngle*/RotFluxAng_(float iqs, float ids, float sPWM, float tRotor,float Omega_r)//added Omega_r
{
	...
	if (wm==0.0)wm=0.000001;
	s_2_=(wm-Omega_r)/wm*(wm-Omega_r)/wm;
...
//	std::cout<<idm<<" idm exp "<<std::endl;
	//a = -Lr/Llr/Tfe; b = 1;//TODO repeated??
	 //ex = exp(a*sPWM);
...
#ifdef FP
s32fp FP_RotFluxAng::/*CalcSlipAngle*/FP_RotFluxAng_(s32fp FP_iqs, s32fp FP_ids, s32fp FP_sPWM, s32fp FP_tRotor,s32fp FP_Omega_r)//added Omega_
{
	// Calculate current slip velocity
	// imr=imr+sPWM/tRotor*(ids-imr);
	// OmegaSlip=(iqs/(tRotor*imr));
		//std::cout<<_imr<<" imr "<<_OmegaSlip<<" omegaslip "<<std::endl;
float wm = FLT_FROMFP( FP_wm ), Omega_r = FLT_FROMFP( FP_Omega_r );
if (wm==0.0)wm=0.00000000001;
	float s_2=(wm-Omega_r)/wm*(wm-Omega_r)/wm;
//std::cout<<wm-Omega_r<<"<-wm-Omega_r"<<wm-Omega_r/wm<<std::endl;
	if (FLT_FROMFP(FP_wm)==0.0)FP_wm = FP_FROMFLT( 0.00000000001 );
	float tempor = FLT_FROMFP(FP_DIV((FP_wm - FP_Omega_r), FP_wm ));
	tempor = tempor * tempor;
	s32fp FP_s_2 = FP_FROMFLT( tempor );// FP_MUL(  FP_DIV((FP_wm - FP_Omega_r), FP_wm ),FP_DIV((FP_wm - FP_Omega_r), FP_wm) );
		//	std::cout<<FLT_FROMFP(FP_wm - FP_Omega_r)<<"<-FP_wm - FP_Omega_r"<<FLT_FROMFP(FP_DIV((FP_wm - FP_Omega_r), FP_wm ))<<std::endl;
	//		std::cout<<wm<<" wm "<<FLT_FROMFP( FP_wm )<<" FP_wm "<<Omega_r<<" Omega_r "<<FLT_FROMFP( FP_Omega_r )<<" FP_Omega_r "<<s_2<<" s_2 "<<FLT_FROMFP( FP_s_2 )<<" FP_s_2 "<<std::endl;

//		std::cout<<s_2<<" r_s_2 "<<std::endl;
		float Rfe=Rm/(tempor+1);
		//s32fp FP_Rfe = FP_DIV(FP_Rm, ( tempor + FP_1));
		s32fp FP_Rfe = FP_FROMFLT( Rfe );
	//	std::cout<<Rfe << " "<< FP_Rfe<<std::endl;
		
//		std::cout<<Rfe<<" Rfe "<<std::endl;
		//float Tfe = (M/Rfe);
		s32fp FP_Tfe = FP_DIV(FP_M,FP_Rfe);
//std::cout<<Tfe<<" Tfe "<<std::endl;
		float T_ro_r = Llr/Rr_est;
		s32fp FP_T_ro_r = FP_DIV( FP_Llr , FP_Rr_est );
		
		
//	std::cout<<T_ro_r<<" T_ro_r "<<std::endl;
		
	s32fp FP_F0= FP_F , FP_idm0 = FP_idm , FP_iqm0 = FP_iqm ;
//	std::cout<<F<<" F "<<idm<<" idm "<<iqm<<" iqm "<<std::endl;
...
//	float k3_2= 1/Tfe*(ids/*+sPWM/2*/)+1/Llr/Tfe*(F/*+k2_1*sPWM/2*/)+(/*??*/wm/*+sPWM/2*/)*(iqm/*+k2_3*sPWM/2*/)-Lr/Llr/Tfe*(idm/*+k2_2*sPWM/2*/);
	...
	//exponential method with losses:
	//float a = -1/T_ro_r; float b = M/T_ro_r;
	s32fp FP_a = FP_DIV(FP__1  , FP_T_ro_r ); s32fp FP_b = FP_DIV( FP_M, FP_T_ro_r );
	
	//float ex = exp(a*sPWM);
	s32fp FP_ex = FP_FROMFLT(exp(FLT_FROMFP(FP_MUL(FP_a,FP_sPWM))));

	FP_F= FP_MUL(FP_MUL(FP_DIV(FP_b ,FP_a) , (FP_ex-FP_1) ) , FP_idm0 )+FP_MUL(FP_ex,FP_F0);
//	std::cout<<F<<" F_exp "<<std::endl;
//TODO FP_Lr, FP_Llr, FP_Tfe
	 FP_a = FP_DIV(-FP_Lr,FP_MUL(FP_Llr,FP_Tfe));  FP_b = FP_1;
	 
	 FP_ex =FP_FROMFLT( exp(FLT_FROMFP(FP_MUL(FP_a, FP_sPWM))));//TODO exp em FP
	
		//idm=  b *1/a*(ex-1)*(ids/Tfe+F0/Llr/Tfe+wm*iqm0)+ex*idm0;
	s32fp	FP_xpto = FP_MUL(FP_DIV(FP_b,FP_a),(FP_ex-FP_1));//TODO calc. inicial da constante FP_FROMFLT(1)
	FP_idm= (FP_MUL(FP_xpto,(FP_DIV(FP_ids,FP_Tfe)+FP_DIV(FP_F0,FP_MUL(FP_Llr,FP_Tfe))+FP_MUL(FP_wm,FP_iqm0))))+FP_MUL(FP_ex,FP_idm0);
	
//	std::cout<<idm<<" idm exp "<<std::endl;
	//TODO REPEATED??a = -Lr/Llr/Tfe; b = 1;
	// ex = exp(a*sPWM);
	
	FP_iqm= FP_MUL(FP_xpto,(FP_DIV(FP_iqs,FP_Tfe)-FP_MUL(FP_wm,FP_idm0)))+FP_MUL(FP_ex,FP_iqm0);
	 
//	std::cout<<iqm<<" iqm ex "<<std::endl;
	
		//OmegaSlip=(M*iqm/(T_ro_r*F));

	FP_OmegaSlip = FP_MUL(FP_M,FP_DIV(FP_iqm,(FP_MUL(FP_T_ro_r,FP_F))));
	
	FP_imr = FP_DIV(FP_F,FP_M);
	
	
	
	//with first order exponential integrator:
	//float
	 FP_a = FP_DIV(/*FP_FROMFLT(-1.0) TODO */FP__1,FP_tRotor);
	  //float 
	  FP_b = FP_DIV(FP_1,FP_tRotor);
	//float
	 FP_ex = FP_FROMFLT(exp(FLT_FROMFP(FP_MUL(FP_a,FP_sPWM))));
	//imr=ex*imr+b *1/a*(ex-1)*(ids);
	...
	while (FP_theta_slip >= FP_2PI ||FP_theta_slip < FP_0)//TODO FP. TODO 0 e 2*PI || 0-PI 
	...
	while (FP_theta_r >= FP_2PI || FP_theta_r < FP_0 )//TODO 0 e 2*PI || 0-PI 
	...
	// Restrict rotor flux angle to a value between 0-2Pi radians
	while ( FP_rot_f_ang >= FP_2PI || FP_rot_f_ang < FP_0 )//TODO 0 e 2*PI || 0-PI 
...
FP_RotFluxAng::FP_RotFluxAng(/*float imr_*/)//TODO necess rfa?? para invocar construc? NEW
{
	...
	FP_idm = FP_FROMFLT(0.00000001);
	FP_iqm = FP_FROMFLT(0.0000001);
	FP_F = FP_FROMFLT(0.00001);
...
s32fp FP_RotFluxAng::/*CalcSlipAngle*/FP_RotFluxAng__(s32fp FP_iqs, s32fp FP_ids, s32fp FP_sPWM, s32fp FP_tRotor,s32fp FP_Omega_r)//added Omega_r
{
...
//		std::cout<<Rfe<<" Rfe "<<std::endl;
		float Tfe = (M/Rfe);
//std::cout<<Tfe<<" Tfe "<<std::endl;
		float T_ro_r = Llr/Rr_est;//TODO not used i think
//	std::cout<<T_ro_r<<" T_ro_r "<<std::endl;
