...
#include <cmath>
#include <math.h>
#include "../include/my_fp_2.h"
#include "../include/sine_core.h"
//#include "variables.hpp"
//#include "Rot_Flux_Ang.hpp"

//--- Defines ------------------------------------------------------------------
#define CONST_SQRT3_  1.732050807568877293527446315059
#define CONST_1_SQRT3 float(1/CONST_SQRT3_)
#define CONST_2_SQRT3 float(2/CONST_SQRT3_)
#define CONST_SQRT3_2 float(CONST_SQRT3_/2)
...
namespace Maths{
     ...
     */
    struct tThreePhase {
        tThreePhase( float ra, float rb, float rc ) : a(ra), b(rb), c(rc) {}
     ...
     */
    struct tTwoPhase {
        tTwoPhase( float ralpha, float rbeta ) : alpha(ralpha), beta(rbeta) {}
     ...
     */
    struct tTwoPhaseDQ {
        tTwoPhaseDQ( float rd, float rq ) : d(rd), q(rq) {}
     ...
     */
    struct tCosSin {
        tCosSin( float radians ) : cos( std::cos(radians) ), sin( std::sin(radians) ) {}

        float  cos;
        float  sin;
    };
	struct FP_tCosSin {
        FP_tCosSin( s32fp radians_ ) : cos_( FP_FROMFLT(((float) (SineCore::Cosine((uint16_t)((FLT_FROMFP(radians_))*(1<<16)/2/PI))))/(1<<15))/*std::cos(radians)*/ ), sin_( FP_FROMFLT(((float) (SineCore::Sine((uint16_t)((FLT_FROMFP(radians_)/*>>CST_DIGITS*/)*(1<<16)/2/PI))))/(1<<15)) /**std::sin(radians)*//**>>CST_DIGITS**/ ) {}
    ...
    inline tTwoPhaseDQ Park( float angle, const tTwoPhase& in ) {
    ...
    inline tTwoPhase InvPark( float angle, const tTwoPhaseDQ& in ) {
    ...
    inline tThreePhase InvClarke( const tTwoPhase& in ) {
        float r1 = -in.alpha/2;
        float r2 = in.beta*CONST_SQRT3_2;
    ...
    inline tTwoPhaseDQ ClarkePark( float angle, const tThreePhase& in ) {
    ...
    inline tThreePhase InvClarkePark( float angle, const tTwoPhaseDQ& in ) {
