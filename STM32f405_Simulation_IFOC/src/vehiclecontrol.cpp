...
bool VehicleControl::CanReceive(uint32_t canId, uint32_t data[2], uint8_t)
{
   ...
   if (canErrors < maxErrors)
   {
      ...
      lastCanRxTime = systick_get_value();/**rtc_get_counter_val() TODO AMRC **/
...
void VehicleControl::GetDigInputs()
{
   ...
   if ((systick_get_value()/**rtc_get_counter_val() TODO AMRC **/ - lastCanRxTime) >= CAN_TIMEOUT && canIoActive)
   {
      canio = 0;
...
float VehicleControl::ProcessUdc()
{
   ...
   if (udcfp < (udcsw / 2) && systick_get_value()/**rtc_get_counter_val() TODO AMRC **/ > PRECHARGE_TIMEOUT && DigIo::prec_out.Get())
   {
      DigIo::err_out.Set();
...
float VehicleControl::GetUserThrottleCommand()
{
   ...
   {
      //500ms timeout
      if ((systick_get_value()/**rtc_get_counter_val() TODO AMRC **/ - lastCanRxTime) < CAN_TIMEOUT)
      {
