...
RotFluxAng::RotFluxAng(/*float imr_*/)//TODO necess rfa?? para invocar construc? NEW
{
	...
	theta_r=0.0;
	theta_slip=0.0;
	imr= 0.0001;//TODO(remove)nao sera necessario se nao voltage compensator
	wm= FLT_FROMFP(1);
}
...
float RotFluxAng::/*CalcSlipAngle*/RotFluxAng_(float iqss, float idss, float sPWM, float tRotor,float Omega_r)//added Omega_r
{
	// Calculate current slip velocity
	imr=imr+sPWM/tRotor*(idss-imr);
	OmegaSlip=(iqss/(tRotor*imr));
...
float RotFluxAng::get_theta_r(){return theta_r;}
float RotFluxAng::get_OmegaSlip(){return OmegaSlip;}
float RotFluxAng::get_theta_slip(){return theta_slip;}
