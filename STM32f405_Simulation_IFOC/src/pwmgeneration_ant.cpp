...
rotorAngle = (( ang.get_theta_r() * 65536 ) / 2 ) / PI;

frq = polePairRatio * wr / 2 / PI +( ang.get_OmegaSlip() / 2 ) / PI ;
angle = polePairRatio * rotorAngle + (ang.theta_slip() * 65536 ) / 2 / PI ;
...
void PwmGeneration::CalcNextAngleConstant(int dir)
{
   ...
   angle += dir * slipIncr;
    #ifdef VehicleSimulation
    frq = ang.get_OmegaSlip() / 2 / PI ;
    angle = (ang.theta_slip() * 65536 ) / 2 / PI ;
   #endif
   if (frq < 0) frq = 0;
}
