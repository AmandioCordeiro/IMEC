...
#define MASSA 3500//(2500.0)//massa veiculo
...
float IDC_med=0.0;
int it=0;
float PowerLoss_med = 0.0;
float PowerLossRotorJoule_med = 0.0;
int it_loss = 0;
int it_lossRotorJoule = 0;

bool quit=false;


float sum_power_recarregada = 0.0;
...
//#ifdef FLOATP TODO RFA
RotFluxAng	FOC::angle;//(0.0);//TODO_ create it in embedded
//#else //TODO TO FP
//RotFluxAng	FOC::angle;//(0.0);//TODO_ create it in embedded
//FP_RotFluxAng	FOC::FP_angle;//(0.0);//TODO_ create it in embedded
//#endif
...
//----------------	the most simple of doing the following, but i dont know if is the more accurate, is measure only IDC, the current flowing from batteries
	...
	float get_ics(){//TODO in real read from sensors or if system simetric and equilibrated: 
		...
		abc_current.c=(InvClarkePark(RotorFluxAngle/*0.0*//*np*theta_r*/,tTwoPhaseDQ(ids,iqs)).c);
...
float PowerLoss = 0.0;
float PowerLossRotorJoule = 0.0;
float idr = 0.0, iqr = 0.0;
...
float torque_g(float Vds,float Vqs,float angle_get_wm){
	float Lls=Ls-M;	
		...
		//TODO JANt
		tThreePhase v_g= InvClarkePark( RotorFluxAngle/*0.0*/, tTwoPhaseDQ( Vds_g , Vqs_g ) );
		vaa_g = ( v_g.a );vbb_g= ( v_g.b );vcc_g = ( v_g.c );
		
		#else
		FP_tThreePhase FP_v_g= FP_InvClarkePark( FLT_FROMFP(FP_RotorFluxAngle), FP_tTwoPhaseDQ( FP_FROMFLT(Vds_g) , FP_FROMFLT(Vqs_g ) ) );

		vaa_g = FLT_FROMFP( FP_v_g.a );vbb_g= FLT_FROMFP( FP_v_g.b );vcc_g = FLT_FROMFP( FP_v_g.c );
		#endif
...
////			}
		//??#ifdef FLOATP
		//tTwoPhaseDQ V_new = ClarkePark(RotorFluxAngle/*0.0*/,tThreePhase (vaa/*_med*/,vbb/*_med*/,vcc/*_med*/));//(((vaa+vaa_g)/2.0),((vbb+vbb_g)/2.0),((vcc+vcc_g)/2.0)));//((vaa_med/*+vaa_g*/)/*2.0*/,(vbb_med/*+vbb_g*/)/*2.0*/,(vcc_med/*+vcc_g*/)/*2.0*/));//TODO
		//#else
		//tTwoPhaseDQ V_new = ClarkePark( FLT_FROMFP( FP_RotorFluxAngle ) , tThreePhase (vaa/*_med*/,vbb/*_med*/,vcc/*_med*/));//(((vaa+vaa_g)/2.0),((vbb+vbb_g)/2.0),((vcc+vcc_g)/2.0)));//((vaa_med/*+vaa_g*/)/*2.0*/,(vbb_med/*+vbb_g*/)/*2.0*/,(vcc_med/*+vcc_g*/)/*2.0*/));//TODO
		//#endif
...
//		std::cout<<sSumPowerLossRotorJoule_2<<" ori "<<std::endl;
		float Rfe = Rm / ( s_2 + 1 ) ;		
...
//Idm = 1/(1/T+Rfe/Lls+Rfe/Llr+Rfe/M)*(Rfe/(Lls*M)*(ids * Lls + M * Idm)+Rfe/(M*Llr)*fdr+Idm/T+(angle_get_wm/*-np*wr*/)*Iqm);
		...
		idr = ( fdr - M * Idm ) / Llr;
		iqr = ( fqr - M * Iqm ) / Llr;
		PowerLossRotorJoule = Rr * ( idr * idr + iqr * iqr ); //TODO ??
		SumPowerLossRotorJoule += PowerLossRotorJoule;
//cout<<"lossrotor"<< SumPowerLossRotorJoule;
		
	/*:2%for mecanic losses and magnetic losses*///TODO ?? t * wr or IDC, case: wr > 0 decreasing, t > 0 == losses
	//if t oposto a direcao de movimento, ou esta a carregar bateria ou a dissipar em perdas
	//else t na mesma direcao de movimento  
	if (  ( t * wr ) < 0  ) sum_power_out = sum_power_out + abs( IDC * VDC ) * T / 3600 ; else sum_power_out = sum_power_out + ( t * wr ) * T / 3600;
	if (  ( t * wr ) < 0  ) sum_power_in = sum_power_in + abs(/*get_torq_L()*/ t * wr ) * T / 3600; else sum_power_in = sum_power_in + abs( IDC * VDC ) * T / 3600;
	if (/*(t*wr)<0 &&*/ (IDC/*VDC*/) < 0 ) sum_power_recarregada = sum_power_recarregada + ( IDC * VDC ) * T / 3600;
	
	//efic considerando somente o motor/gerador??:
	//sum_power_mod_mec += abs( t * wr ) * T / 3600 ;
	//sum_power_mod_elec += abs( IDC * VDC ) * T / 3600 ;
	
	sum_power_trac += t * wr * T / 3600;fTorque<<"sum power trac.: "<<sum_power_trac<<" wh"<<endl;
	sum_power_battery += IDC * VDC * T / 3600;fTorque<<"battery energy consumed : "<<sum_power_battery<<"battery energy recharged : "<<sum_power_recarregada<<" wh"<<endl;
	
//	fTorque<<" efficiency trac/bat "<<sum_power_trac/sum_power_battery<<endl;
		//if (n%2 == 0 /*&& n != 0*/){ 
			fTorque<<"Vds: "<<Vds<<" Vqs: "<<Vqs<<" angle_get_wm:"<<angle_get_wm<<endl;

//if ( t * wr < 0.0 ) PowerLoss = -t * wr  - -VDC * IDC;else PowerLoss =  VDC * IDC - t * wr ;//TODO see if makes sense, logic
PowerLoss =  VDC * abs( IDC ) - abs( t ) * abs( wr ) ;
fTorque<< "PowerLoss" << PowerLoss << endl;
		...
		//};	
	float coef_loss = 0.025 ; //slip
	//wr(sentido do mov.)
	// setpoint do torque ou wm < wr ...
	
	//if ( wr > 0 angle_get_wm > np * wr ) //perdas_mec_positivas, histerese positiva, slip positiva
// if ( wr > 0  && foc_.get_torq_setpoint() < 0 /*angle_get_wm < np * wr */) coef_loss *= -1.0;//perdas_mec_neg, histerese neg, slip neg?
...
float torque(float vas,float vbs,float vcs){
		//fTorque<<" FP VDQ.d "<<FLT_FROMFP(FP_ClarkePark(0.0/*RotorFluxAngle*//*np*theta_r*/,FP_tThreePhase(FP_FROMFLT(vas),FP_FROMFLT(vbs),FP_FROMFLT(vcs))).d)<</*alpha*/"FP VDQ.q "<<FLT_FROMFP(FP_ClarkePark(0.0/*RotorFluxAngle*//*np*theta_r*/,FP_tThreePhase(FP_FROMFLT(vas),FP_FROMFLT(vbs),FP_FROMFLT(vcs))).q)/*beta*/<<" FLT_FROMFP(FOC::angle.FP_get_wm()) "<<FLT_FROMFP(FOC::angle.get_wm())<<std::endl;
//fTorque<<" VDQ.d "<<ClarkePark(0.0/*RotorFluxAngle*//*np*theta_r*/,tThreePhase(vas,vbs,vcs)).d/*alpha*/<<" VDQ.q "<<ClarkePark(0.0/*RotorFluxAngle*//*np*theta_r*/,tThreePhase(vas,vbs,vcs)).q/*beta*/<<"angle.get_wm "<<FLT_FROMFP(FOC::angle.get_wm())<<std::endl;
		#ifdef FLOATP
		return torque_g(ClarkePark(RotorFluxAngle/*0.0*//*np*theta_r*/,tThreePhase(vas,vbs,vcs)).d/*alpha*/,ClarkePark(RotorFluxAngle/*0.0*//*np*theta_r*/,tThreePhase(vas,vbs,vcs)).q/*beta*/, (FOC::angle.get_wm()));//test//
		#else
		return torque_g(ClarkePark(RotorFluxAngle/*0.0*//*np*theta_r*/,tThreePhase(vas,vbs,vcs)).d/*alpha*/,ClarkePark(RotorFluxAngle/*0.0*//*np*theta_r*/,tThreePhase(vas,vbs,vcs)).q/*beta*/, /*TODO FPFLT_FROMFP*/(FOC::angle.get_wm()));//test//
		#endif
};
...
float get_wr(float va,float vb,float vc/*float torq_g,float torq_L*/)
{
		...
		if ( driving_cycle_mode == true  ){
			if ( wr * np < 380/*380*/ && gear_ == 2 /*&& velocidade < velo_ant_red * 0.9 *//*|| velocidade < Wc && velocidade < velo_ant_red * 0.9*/){
			...
			if ( wr * np > 1600/*1300*//*835*//*Wc*0.92*/ && gear_ == 2 /*&& velocidade > velo_ant_red * 1.1*/ ){
			//	velo_ant_red = velocidade;
				fVel<<" > Wn*1.4 "<<endl;
				gear_++;
				T_G_R = T_G_R_[ (gear_ - 1) ];
			}
			if ( wr * np < 350 && gear_ == 3 /*&& velocidade < velo_ant_red * 0.9 *//*|| velocidade < Wc && velocidade < velo_ant_red * 0.9*/){
			//	velo_ant_red = velocidade;
				fVel<<" < Wn*1.3 "<<endl;
				gear_--;
				T_G_R = T_G_R_[ (gear_ - 1) ];
			}
			//if ( wr * np > /*735*/980/*Wc*0.92*/ && gear_ == 3 /*&& velocidade > velo_ant_red * 1.1*/ ){
			////	velo_ant_red = velocidade;
			//fVel<<" > g4"<<endl;
			//gear_++;
			//	T_G_R = T_G_R_[ (gear_ - 1) ];
			//}
			//if ( wr * np < 380 && gear_ == 4 /*&& velocidade < velo_ant_red * 0.9 *//*|| velocidade < Wc && velocidade < velo_ant_red * 0.9*/){
			//	velo_ant_red = velocidade;
			//	fVel<<" < g "<<endl;
			//	gear_--;
			//	T_G_R = T_G_R_[ (gear_ - 1) ];
...
//				w_ref=0;//jan 2019 ;w_ref *T_G_R/T_G_R_ant;/*end_f*1000/3600*T_G_R/R*/;
		...
		//g//if (e==true && velocidade*R/T_G_R*3.6 < 19/*26*/){//TODO para bat 145 e nao 300 dividi por 1.5 a velocidade de mudança pois wn e wc é metade
		//g//		T_G_R = (4.313 /*2.33 1.436 1 0.789*/*4.1/*1.9*/);
		//g//	if (T_G_R_ant!=T_G_R){
		//g//		T_G_R_b_clutch=T_G_R_ant;
				//wr = wr *T_G_R/T_G_R_ant;
		//g//		w_ref_ant=w_ref;
		//g//		w_ref=0;//jan 2019 ;w_ref *T_G_R/T_G_R_ant;/*end_f*1000/3600*T_G_R/R*/;
				
		//g//		clutch_time=CLUTCH_TIME*1/T;
		//g//		velocidade=velocidade*T_G_R/T_G_R_ant;
		//g//		T_G_R_ant=T_G_R;
		//g//		n_rtc=0;
				//FOC::angle.reset();
			//g//	}
		...
		//g//	}
		//g//if (e==true && velocidade*R/T_G_R*3.6 > 22/*30*/ && velocidade*R/T_G_R*3.6 < 35/*66*/ )
			//g//{T_G_R = (/*4.313*/ 2.33/*1.436 1 0.789*/*4.1/*1.9*/);
			//g//if (T_G_R_ant!=T_G_R){
				//wr = wr *T_G_R/T_G_R_ant;
			//g//	w_ref_ant=w_ref;
			...
			//g//	}
		//g//	}
		//g//if (e==true && velocidade*R/T_G_R*3.6 > 37.5/*30*/ && velocidade*R/T_G_R*3.6 < 55.8/*66*/ )
			//g//{T_G_R = (/*4.313*/ 1.436/*1.436 1 0.789*/*4.1/*1.9*/);
			//g//if (T_G_R_ant!=T_G_R){
				//wr = wr *T_G_R/T_G_R_ant;
				//g//w_ref_ant=w_ref;
			//g//	w_ref=0;//jan 2019 ;w_ref *T_G_R/T_G_R_ant;/*end_f*1000/3600*T_G_R/R*/;
			...
			//g//	}
		//g//	}
		//g//if (e==true && velocidade*R/T_G_R*3.6 > 58.3/*30*/ && velocidade*R/T_G_R*3.6 < 81.5/*66*/ )
		//g//	{T_G_R = (/*4.313 1.436*/ 1/* 0.789*/*4.1/*1.9*/);
		//g//	if (T_G_R_ant!=T_G_R){
				//wr = wr *T_G_R/T_G_R_ant;
		//g//		w_ref_ant=w_ref;
		//g//		w_ref=0;//jan 2019 ;w_ref *T_G_R/T_G_R_ant;/*end_f*1000/3600*T_G_R/R*/;
	...
	//g//	if (e==true && velocidade*R/T_G_R*3.6 > 83.505/*70*/ )
		//g//	{T_G_R = (/*4.313 2.33 1.436 1*/ 0.789*4.1/*1.9*/);
			//g//if (T_G_R_ant!=T_G_R){
				//wr = wr *T_G_R/T_G_R_ant;
		//g//		w_ref_ant=w_ref;
		...
		if( driving_cycle_mode ==true){//if driving cycle:
			if (clutch_time>0){
				clutch();
			...
			if (T_G_R_ant!=T_G_R){
				//wr = wr *T_G_R/T_G_R_ant;
				w_ref_ant=w_ref;
				flag_no_torque_motor = true;//w_ref=0;//w_ref *T_G_R/T_G_R_ant;/*end_f*1000/3600*T_G_R/R*/;
			...
			//	}
				...
				if (T_G_R==0){
					...
					clutch();
						...
						//float wr_1 = wr;
						wr=wr+(Torq_net)*T/(J+J2/*R*R*MASSA/(T_G_R*T_G_R)*/);
						float torque_J = ( wr - velocidade /*wr_1*/ ) / T * ( J + J2 );//TODO remove, just log
						fTorque<<"Te: "<<Te<<"torq_resistivo total"<<torq_L+torque_J<<"torq_load "<< torq_L<<"torque_J"<<torque_J<<" w_ref "<<w_ref<<" wr "<<wr<<endl;
						velocidade=wr;
					}
			}	
			}
			//if (n%2 == 0 /*&& n != 0*/){ 
				//fTorque<<"Te: "<<Te<<"torq_resistivo "<<torq_L+torque_J<<" w_ref "<<w_ref<<" wr "<<wr<<endl;
			//}
...
float get_w_r(){
	...
	return wr;};//TODO implement in REAL
...
//	long c=0;
	...
	string line;
	int ii = 0;//4;//here, 0 to full test 4 ece +1 eudc.. 4 for eudc only
	bool primeiro=true;
	int gear=1;
	
	float IDC_med_ = 0.0;
	float PowerLoss_med_ = 0.0;
	float PowerLossRotorJoule_med_ = 0.0;

	enable_raw_mode();
	
	while (quit==false)
	{
	//Rrest if (Rr_est<0.0003)quit=true;//TODO remove this is in test??
		if (kbhit())
		{
			o=getchar();
			switch(o){					
	...
	////tcflush(0, TCIFLUSH);cin>>imrreff;enable_raw_mode();/*fflush(stdin);*/break;
				case 's' :cout<<"enter speed:"; driving_cycle_mode = false;/*e-> to set by driving cycle or by speed*//*if(flag==true){flag=false;break;};*/disable_raw_mode();
	...
	tcflush(0, TCIFLUSH);cin>>load_s;enable_raw_mode(); vehicle_load_mode =false; driving_cycle_mode = false;/*fflush(stdin);*/break; 
	...
	//tcflush(0, TCIFLUSH);char f='p';while(f != ' '){cin>>f;cout<<f;};enable_raw_mode();/*fflush(stdin);*/break;
				...
				case 'v' :cout<<"set by vehicle load,enter road gradiente(ex. 0.0 <-> 0.2): "; vehicle_load_mode = true; driving_cycle_mode = false;/*break;*///inicial p=true
	...
	//tcflush(0, TCIFLUSH);cin>>pl;enable_raw_mode();/*fflush(stdin);*/break;
				...
				case 'D' :disable_raw_mode();tcflush(0, TCIFLUSH);cout<<"running driving cycle"<<endl;run=true; driving_cycle_mode = true;enable_raw_mode();break;//set by driving_cycle
		...
		//TODO remove following at end
			...
			else if ( ( vehicle_load_mode || driving_cycle_mode ) && get_w_r()!=0) {
		...
		//TODO remove above at end
			...
			if (ii < 4 && driving_cycle_mode == true && driving_cycle.is_open())
			{				
						...
						bb = strtod (s_line/*szOrbits*/, &pEnd);
						end_f = strtod (pEnd, NULL);
						fVel<<" (bb+196*"<< ii <<"): "<<(bb + 196* ii)/*<<" b e end :"<<b<<end*/<<endl;
						////
						//if(b==62){
						//	
						...
						else if ((n*T) == ( bb +196* ii)) 	
						{	w_ref=end_f*1000/3600*T_G_R/R;
							getline(driving_cycle,line);
						
						}
						fTorque<<"w_ref "<<w_ref<<endl;
						//if (b==25)quit=true;//test 
			}
			//extra urban drive cycle
			else if ( driving_cycle_mode ==true)
				{
				if (primeiro){
				...
				fVel<<"  line_   "<<line<<"reference velocity_ (km/h)"<<endl;
				while(line.length() > 13 )getline(driving_cycle,line);	
				...
				end_f = strtod (pEnd, NULL);
				fVel<< (n * T)<< " nt "<<bb<<endl;
				if (( n * T )  == /*bb*/ ( bb +196*ii) )	//here to run only eudc change to b only
				{	w_ref=end_f*1000/3600*T_G_R/R;
					getline(driving_cycle,line);
				
				}
				if ( bb == /*199*/400)quit=true;//here to run only ece change to 175
				};	
			...
			/*here*/
			#ifdef FLOATP
			iaa_p=/*FLT_FROMFP_*/get_ias();//used to calc cosphi
			#else
...
/////____
	...
	FP_VDC = FP_FROMFLT(VDC);
			...
			Vmax = VDC/CONST_SQRT3_;//TODO remove in end
			#ifndef FLOATP
		...
		//	if (n%4 == 0 /*&& n != 0*/)
			{ 
				T = T_8khz;// 1.0 / ( 1 << 13 ) ;// 16 khz T*2.0; defined another variable TT double freq.
				#ifdef FLOATP
				...
				#endif
				T = T_16khz;//1.0 / ( 1 << 14 );// T/2.0; 8 khz
...
fTorque<<" vaa_n:"<<vaa<<" vbb_n:"<<vbb<<" vcc_n:"<<vcc<<endl<<"IDC_previous: "<<IDC<<" VDC: "<<VDC<<" "<<endl;
	...
	{		//********** used in simulation to calc cos_phi
			...
			{ 
					...
					#else
					fTorque<<"power iaa*vaa+ibb*vbb+icc*vcc: "<<FLT_FROMFP_get_ias()*vaa+/*InvClarkePark(RotorFluxAngle,IDQ)*/FLT_FROMFP_get_ibs()*vbb+FLT_FROMFP_get_ics()*vcc<<endl;
					fTorque<<" ia "<<FLT_FROMFP_get_ias()<<" ib "<<FLT_FROMFP_get_ibs()<<" ic "<<FLT_FROMFP_get_ics()<<endl;
					fTorque<</*" FP_*/"ia "<<FLT_FROMFP_get_ias()<</*" FP_*/"ib "<<FLT_FROMFP_get_ibs()<</*" FP_*/"ic "<<FLT_FROMFP_get_ics()<<endl;
					#endif
					//fTorque<<"vaa: "<<vaa<<" iaa: "<<abc_current.a<<endl;
					
					//TOFDO FP AND FLOATP?? sfData_va_ia<<" "<<FIXED_FLOAT(n*T)<<" "<<vaa<<" "<<FLT_FROMFP_get_ias()<<" "<<FLT_FROMFP_get_ibs()<<" "<<FLT_FROMFP_get_ics()<<endl;
					...
					( FLT_FROMFP_get_ias() ) < 0 ) {
						...
						#else
						fTorque<<"iaa_p"<<iaa_p<<"iaa"<<FLT_FROMFP_get_ias()<<" phi: "<<cos_phi_a<<" cos_phi: "<<cos(cos_phi_a)<<endl;}
						#endif
			//**********
	}	
...
//_____________
		...
		if (it_loss< 1*1.0 / T ){
			it_loss++;
			PowerLoss_med += PowerLoss;
			//IDC_med_ = (IDC_med/it);
			//IDC_med/=2;
			
		}else{
			PowerLoss_med_ = (PowerLoss_med/it_loss);
			it_loss = 0;
			PowerLoss_med = 0.0;		
			}
	//	if (n%2 ==0 )
			fTorque<<" PowerLoss med "<<PowerLoss_med_<<endl;
		//::::::::
//_____________
		//:::::::IDC medio
		
		if (it_lossRotorJoule< 1*1.0 / T ){
			it_lossRotorJoule++;
			PowerLossRotorJoule_med += PowerLossRotorJoule;
			//IDC_med_ = (IDC_med/it);
			//IDC_med/=2;
			
		}else{
			PowerLossRotorJoule_med_ = (PowerLossRotorJoule_med/it_lossRotorJoule);
			it_lossRotorJoule = 0;
			PowerLossRotorJoule_med = 0.0;		
			}
	//	if (n%2 ==0 )
			fTorque<<" PowerLossRotorJoule med "<<PowerLossRotorJoule_med_<<endl;
		//::::::::
