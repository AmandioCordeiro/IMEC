...
#define MASSA 2500//(3500.0)//massa veiculo
...
//fTorque<<" VDQ.d "<<ClarkePark(0.0/*RotorFluxAngle*//*np*theta_r*/,tThreePhase(vas,vbs,vcs)).d/*alpha*/<<" VDQ.q "<<ClarkePark(0.0/*RotorFluxAngle*//*np*theta_r*/,tThreePhase(vas,vbs,vcs)).q/*beta*/<<"angle.get_wm "<<FLT_FROMFP(FOC::angle.get_wm())<<std::endl;
		#ifdef FLOATP
		return torque_g(ClarkePark(0.0/*RotorFluxAngle*//*np*theta_r*/,tThreePhase(vas,vbs,vcs)).d/*alpha*/,ClarkePark(0.0/*RotorFluxAngle*//*np*theta_r*/,tThreePhase(vas,vbs,vcs)).q/*beta*/, (FOC::angle.get_wm()));//test//
		#else
		return torque_g(ClarkePark(0.0/*RotorFluxAngle*//*np*theta_r*/,tThreePhase(vas,vbs,vcs)).d/*alpha*/,ClarkePark(0.0/*RotorFluxAngle*//*np*theta_r*/,tThreePhase(vas,vbs,vcs)).q/*beta*/, FLT_FROMFP(FOC::FP_angle.FP_get_wm()));//test//
		#endif
};
...
float get_wr(float va,float vb,float vc/*float torq_g,float torq_L*/){
		...
		if ( e == true  ){
			if ( wr * np < 380 && gear_ == 2 /*&& velocidade < velo_ant_red * 0.9 *//*|| velocidade < Wc && velocidade < velo_ant_red * 0.9*/){
			//	velo_ant_red = velocidade;
				fVel<<" < Wn*0.66 "<<endl;
				gear_--;
				T_G_R = T_G_R_[ (gear_ - 1) ];
			}
			if ( wr * np > 835/*Wc*0.92*/ && gear_ == 1 /*&& velocidade > velo_ant_red * 1.1*/ ){
			//	velo_ant_red = velocidade;
				fVel<<" > Wn*0.7 "<<endl;
				gear_++;
				T_G_R = T_G_R_[ (gear_ - 1) ];
			}
			if ( wr * np > 835/*Wc*0.92*/ && gear_ == 2 /*&& velocidade > velo_ant_red * 1.1*/ ){
			//	velo_ant_red = velocidade;
				fVel<<" > Wn*1.4 "<<endl;
				gear_++;
				T_G_R = T_G_R_[ (gear_ - 1) ];
...
//				w_ref=0;//jan 2019 ;w_ref *T_G_R/T_G_R_ant;/*end_f*1000/3600*T_G_R/R*/;
	...
	//g//	if (e==true && velocidade*R/T_G_R*3.6 > 83.505/*70*/ )
		...
		if(e==true){//if driving cycle:
			...
			if (n%2 == 0 /*&& n != 0*/){ 
				fTorque<<"Te: "<<Te<<"torq_L: "<<torq_L<<" w_ref "<<w_ref<<" wr "<<wr<<endl;
			}
