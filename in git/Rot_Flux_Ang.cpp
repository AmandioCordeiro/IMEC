...
double SlipAngle::/*CalcSlipAngle*/RotFluxAng(double iqs, double ids, double sPWM, double tRotor,double Omega_r)//added Omega_r
{
	...
	while (rot_f_ang >= (2*PI)||rot_f_ang<0)//TODO 0 e 2*PI || -2*PI e 2PI 
	{       if(rot_f_ang>=(2*PI))
...
double SlipAngle::get_imr(){return imr;};//TODO(remove)nao sera necessario se nao voltage compensator 
double SlipAngle::get_rfa(){return rot_f_ang;};
