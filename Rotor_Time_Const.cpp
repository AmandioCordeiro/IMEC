...
//float a0_2[14];float a0_2b2[19];float a0a1[13];float a0a1b1[20];float a1_2[12];float a1_2b0[20];
	...
	void r_k2::adic(double *pol1, unsigned int dim1, double *pol2, unsigned int dim2){//dim2<dim1vv
		for(unsigned int i=0;i<=dim2;i++)pol1[i]=pol1[i]+pol2[i];
	};
	void r_k2::inv(double *pol_fin, unsigned int dimo){
		for(unsigned int i=0;i<=(dimo);i++)pol_fin[i]=-pol_fin[i];
	};
//	public:
 r_k2::r_k2/*Y_W_*/():Y_W_(){for(int j=0;j<7;j++){a1[j]=0.0;}for(int j=0;j<8;j++){a0[j]=0.0;}for(int j=0;j<7;j++){b2[j]=0.0;}for(int j=0;j<8;j++){b1[j]=0.0;}for(int j=0;j<9;j++){b0[j]=0.0;}for(int j=0;j<15;j++){a0_2[j]=0.0;}for(int j=0;j<21;j++){a0_2b2[j]=0.0;}for(int j=0;j<14;j++){a0a1[j]=0.0;}for(int j=0;j<21;j++){a0a1b1[j]=0.0;}for(int j=0;j<13;j++){a1_2[j]=0.0;}for(int j=0;j<21;j++){a1_2b0[j]=0.0;}/*a1[7]={0.0};a0[8]={0.0};b2[6]={0.0};b1[8]={0.0};b0[9]={0.0};a0_2[15]={0.0};a0_2b2[20]={0.0};a0a1[14]={0.0};a0a1b1[21]={0.0};a1_2[13]={0.0};a1_2b0[21]={0.0};*/};
 r_k2::~r_k2(){};
	void r_k2::update_r_k2(){
		...
		a0[0]=(Rw[6][7]+Rw[7][6]);for(int i=0;i<8/*k2.size()*/;i++){cout<<"a0:"<<a0[i]<<endl;};cout<<"aaaaaaa"<<endl;
		...
		b1[0]=(/*4*Rwy[5]*/-4*(Rw[6][7]+Rw[7][6]));for(int i=0;i<8/*k2.size()*/;i++){cout<<"b1:"<<b1[i]<<endl;};cout<<"bbb111"<<endl;
		...
		adic(a0a1b1,20,a0_2b2,20);
		adic(a0a1b1,20,a1_2b0,20);for(int i=0;i<21/*k2.size()*/;i++){cout<<"a0a1b1:"<<a0a1b1[i]<<endl;};cout<<"iiiiiii"<<endl;//r_k2=a0a1b1
	};
//};
...
for(int i=0;i<k2.size();i++){cout<<"kkkkkkkkkkkkkkkkkkkkk222222222222222:"<<k2[i]<<endl;};cout<<"iiiiiii"<<endl;
	...
	/*h00=                                                           
      2/k2^2*(rw[6][6]/k2^2+rw[6][5]/k2+k2rw[6][3]+rw[6][0])+2/k2*(rw[5][6]/k2^2+rw[5][5]/k2+k2*rw[5][3]+rw[5][0])            
	  +2*k2*(rw[3][6]/k2^2+rw[3][5]/k2+k2*rw[3][3]+rw[3][0])+2 rw[0][6]/k2^2+2 rw[0][5]/k2+2*k2*rw[0][3] +2*rw[0][0];
      */                               
                                    
	 h01=
	-2*(-2/3*Rwy[6]/pow(k2[k2if],3)-Rwy[5]/pow(k2[k2if],2)+Rwy[3])
	...
	+1/k2[k2if]*(-2*Rw[5][7]/pow(k2[k2if],3)-2*k1[k1jf]*Rw[5][6]-((k1[k1jf])/pow(k2[k2if],2))*Rw[5][5]-Rw[5][4]/pow(k2[k2if],2)+k1[k1jf]*Rw[5][3]+2*k2[k2if]*Rw[5][2]+Rw[5][1])
	...
	-4/pow(k2[k2if],3)*(-2/pow(k2[k2if],3)*Rw[7][7]-2/pow(k2[k2if],3)*Rw[7][6]-k1[k1jf]/pow(k2[k2if],2)*Rw[7][5]-Rw[7][4]/pow(k2[k2if],2)+k1[k1jf]*Rw[7][3]+2*k2[k2if]*Rw[7][2]+Rw[7][1])
