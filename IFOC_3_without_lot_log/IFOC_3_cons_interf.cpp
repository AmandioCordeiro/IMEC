...
//Idm = 1/(1/T+Rfe/Lls+Rfe/Llr+Rfe/M)*(Rfe/(Lls*M)*(ids * Lls + M * Idm)+Rfe/(M*Llr)*fdr+Idm/T+(angle_get_wm/*-np*wr*/)*Iqm);
	...
	if (/*(t*wr)<0 &&*/ (IDC/*VDC*/)<0) sum_power_out += abs(IDC*VDC)*T/3600 ; else sum_power_out += (t*wr)*T/3600;
	if (/*(t*wr)<0 &&*/ (IDC/*VDC*/)<0) sum_power_in += abs(/*get_torq_L()*/t*wr)*T/3600; else sum_power_in += (IDC*VDC)*T/3600;
	//sum_power_trac += t*wr;
	sum_power_battery += IDC*VDC*T/3600;

		if (n%2 == 0 /*&& n != 0*/){ 
		fTorque<<"battery energy consumed : "<<sum_power_battery<<" wh"<<endl;

			//fTorque<<" sum power in: "<<sum_power_in<<"Vds: "<<Vds<<" Vqs: "<<Vqs<<" angle_get_wm:"<<angle_get_wm<<endl;
	...
	t=0.975/*<-losses mec., histerese, slip*/*(3.0/2.0*M*np/Lr/roLs*(fdr*(fqs)-fqr*(fds)));//torque generated by the tension applied
...
float torque(float vas,float vbs,float vcs){
		return torque_g(ClarkePark(0.0/*RotorFluxAngle*//*np*theta_r*/,tThreePhase(vas,vbs,vcs)).d/*alpha*/,ClarkePark(0.0/*RotorFluxAngle*//*np*theta_r*/,tThreePhase(vas,vbs,vcs)).q/*beta*/,FOC::angle.get_wm());
};
...
//	long c=0;
	...
	//tcflush(0, TCIFLUSH);cin>>pl;enable_raw_mode();/*fflush(stdin);*/break;
		...
		//TODO remove above at end
			...
			//l//fwref<<FIXED_FLOAT(n*T)<<"sec.; motor rotor reference velocity: "<<w_ref<<endl;
			//l//fload<<FIXED_FLOAT(n*T)<<"sec.; Torque load: "<<get_torq_L()<<endl;
			...
			{				
						...
						//l//fVel<<"  line "<<line<<"reference velocity (km/h)"<<endl;
						...
						//l//fVel<<" (b+196*"<<i<<"): "<<(b+196*i)/*<<" b e end :"<<b<<end*/<<endl;
						////
						//if(b==62){
						//	
