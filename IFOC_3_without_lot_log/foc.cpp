...
bool FOC::exc_v_PI(){	//inside a while
				float qqq = torque_control.get_pid_result();
				//fTorque<<"torque current error: "<<torque_control.get_current_error()<<"torque pid result: "<<qqq<<endl;//TODO remove at end
			...
			////	};
			current_control_q.calc_pid();
			float qq = current_control_q.get_pid_result();
					//	fTorque<<"current_control_q current error: "<<current_control_q.get_current_error()<<"current_control_q pid result: "<<qq<<endl;//TODO remove at end
