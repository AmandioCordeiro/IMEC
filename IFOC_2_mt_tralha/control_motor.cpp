...
//float Tr_calc_gl=Trrr;
float m_Tr_calc_gl=Trrr;//TODO insert at the end Tr motor value

float M=0.001500;//0.050382;/*0.51233;*///0.0117//0.069312//mutual inductance
float Ls=(0.000140+M);//0.051195;/*(0.00845+M);*///0.014//0.07132

float Lr=(0.000140+M);//0.052571;/*(0.00515+M);*///0.014//0.07132
#define Lps Ls-M*M/Lr
float J=(0.071);//TODO:?? 1pv5135-4ws14: 0.071
float ro=(1.0-(M*M)/(Ls*Lr));//0.999863//1-M*M/(Ls*Lr)//alterar nome

float VDC=300.0;//need actualization in pratice in function get_VDC( -Ifoc
float Vmax=VDC/CONST_SQRT3_;
/*#define*/float Wn = (Vmax/(Ls*sqrt(Idn*Idn+ro*ro*(Imax*Imax-Idn*Idn))));
/*#define*/float Wc = /*0.98**/Vmax/(Imax*Ls)*sqrt((ro*ro+1.0)/(2.0*ro*ro));//TODO acrescentei 0.9??
#define Kt (3.0/2.0*np*M*M/Lr)//TODO atencao pois M aparece como nome de 2 variaveis,repetido

/*#define*/float Rm=650.0; //TODO, nao funcionou c define nao sei pk, talvez dpois d inserir .0 ja deia, como esta agora
/*#define*/float Llr=(Lr-M);//0.0021891 //TODO verificar Llr=Lr-M;
#define CLUTCH_TIME 0.5//0.13//0.5
int clutch_time=-1;
float roLs=(ro*Ls);//0.013998//ro*Ls

tTwoPhase v_(0.0,0.0);
float velocidade=0;//vel. clutch
float T = 0.000125;//0.0001-10khz//TODO: PWM periode 0.000125-8khz  
...
float vaa,vbb,vcc=0.0;
//float VDQ_alpha_esc,VDQ_beta_esc;

float iaa_p,vaa_p,cos_phi,two_phi=0.0,desc_p=0.0;
...
float const_VDQ_d=0.0;
float const_VDQ_q=0.0;
tTwoPhaseDQ VDQ_ant_1(0.0,0.0);
tThreePhase v(0.0,0.0,0.0);
float T0,T1,T2;//2-errado pois T1+T2 estava a dar > k T. 1-T0 indica o tempo por periodo k esta desligado
int a1,b1,c1,a2,b2,c2;//interruptores da ponte trifásica, 1- first time T1, 2- second time T2
float pwm_a=0.0,pwm_b=0.0,pwm_c=0.0;//pwm_a- racio time of T that superior leg of a is open, inferior leg complementar
...
float torque_clutch=0.0;

float Vds_T_1=0.0;
float Vds_T=0.0;
float Vqs_T_1=0.0;
float Vqs_T=0.0;
float angle_get_wm_T_1=0.0;
float angle_get_wm_T=0.0;
float wr_T_1=0.0;
float wr_T=0.0;
//speed up code
...
void svpwm(control_loops & control_){//tTwoPhase- bifasico, tendo como referencia o estator
	...
	if (1==1&& ((VDQ.d*VDQ.d+VDQ.q*VDQ.q)) > max_mod_v_ref*max_mod_v_ref ){
		...
		fTorque<<"exceeded by:"<<sqrt(VDQ.d*VDQ.d+VDQ.q*VDQ.q)<<endl;
	...
	T1T2<<" T1:"<<FIXED_FLOAT_L(T1)<<" a1:"<<a1<<" b1:"<<b1<<" c1:"<<c1<<"   T2:"<<FIXED_FLOAT_L(T2)<<" a2:"<<a2<<" b2:"<<b2<<" c2:"<<c2<<"    T0:"<<FIXED_FLOAT_L(T0)<<endl;
	T1T2<<" pwm_a:"<<pwm_a<<" pwm_b:"<<pwm_b<<" pwm_c:"<<pwm_c<<endl;
	fTorque<<" T1:"<<FIXED_FLOAT_L(T1)<<" a1:"<<a1<<" b1:"<<b1<<" c1:"<<c1<<"   T2:"<<FIXED_FLOAT_L(T2)<<" a2:"<<a2<<" b2:"<<b2<<" c2:"<<c2<<"    T0:"<<FIXED_FLOAT_L(T0)<<endl<<" pwm_a:"<<pwm_a<<" pwm_b:"<<pwm_b<<" pwm_c:"<<pwm_c<<endl;
v_=v;//TODO remove in embedded

};

////!!!!!!!!!!!!!!float dy_nt(float &y1, float &y_1);//y1 means y[n+1] 
	//return dynt=(y1-y_1)/(2*per);

////!!!!!!!-tirar isto final-float d2y_nt(float &y1, float &y,float &y_1);//derived can only be obtained  when n>=? 
//extern float ro;//0.999863//1-M*M/(Ls*Lr)//alterar nome
//extern float B;//M/(ro*Ls*Lr)
//extern float roLs;//0.013998//ro*Ls
//extern float MB;//0.69851//B*M
//extern float one_p_MB;//1.69851//1+MrB
float Lsro=(ro*Ls);//0.00081261
float Lrro=(ro*Lr);//0.0021891
#define R 0.34 //whell radius
#define MASSA (3500.0)//massa veiculo
/*#define*/float T_G_R = (4.313/*2.33*//*1.436 1 0.789*/*4.1/*1.9*/); //total gear racio. (gear_box*final_gear*"reduction_low_ratio")
...
float motor::torque_z(float Vds,float Vqs,float angle_get_wm){
	...
	float Lls=Ls-M;
		...
		//float fds_k1=T/(1/T+Rs/Lls)*(Vds+fds/T+/*np*wr*/angle_get_wm*fqs+Rs*M/Lls*Idm);
		//float fqs_k1=T/(1/T+Rs/Lls)*(Vqs+fqs/T-/*np*wr*/angle_get_wm*fds+Rs*M/Lls*Iqm);
		//float fdr_k1=T/(1/T+Rr/Llr)*(fdr/T+(angle_get_wm-np*wr)*fqr+M*Rr/Llr*Idm);
		//float fqr_k1=T/(1/T+Rr/Llr)*(fqr/T-(angle_get_wm-np*wr)*fdr+M*Rr/Llr*Iqm);
		//float Idm_k1=T/(1/T+Rfe/Lls+Rfe/Llr+Rfe/M)*(Rfe/(Lls*M)*fds+Rfe/(M*Llr)*fdr+Idm/T+(angle_get_wm/*-np*wr*/)*Iqm);//(-Lr/Llr*Idm+ids+fdr/Llr+angle_get_wm/*np*wr*/*M/Rfe*Iqm)/M*Rfe;
		//float Iqm_k1=T/(1/T+Rfe/Lls+Rfe/Llr+Rfe/M)*(Rfe/(Lls*M)*fqs+Rfe/(M*Llr)*fqr+Iqm/T-(angle_get_wm/*-np*wr*/)*Idm);//(-Lr/Llr*Iqm+iqs-angle_get_wm/*np*wr*/*M/Rfe*Idm)/M*Rfe;

		//float fds_k2=T/(1/T+Rs/Lls)*((Vds+T/2)+(fds+fds_k1/2)/T+/*np*wr*/angle_get_wm*(fqs+fqs_k1/2)+Rs*M/Lls*(Idm+Idm_k1/2));
		//float fqs_k2=T/(1/T+Rs/Lls)*((Vqs+T/2)+(fqs+fqs_k1/2)/T-/*np*wr*/angle_get_wm*(fds+fds_k1/2)+Rs*M/Lls*(Iqm+Iqm_k1/2));
		//float fdr_k2=T/(1/T+Rr/Llr)*((fdr+fdr_k1/2)/T+(angle_get_wm-np*wr)*(fqr+fqr_k1/2)+M*Rr/Llr*(Idm+Idm_k1/2));
		//float fqr_k2=T/(1/T+Rr/Llr)*((fqr+fqr_k1/2)/T-(angle_get_wm-np*wr)*(fdr+fdr_k1/2)+M*Rr/Llr*(Iqm+Iqm_k1/2));
		//float Idm_k2=T/(1/T+Rfe/Lls+Rfe/Llr+Rfe/M)*(Rfe/(Lls*M)*(fds+fds_k1/2)+Rfe/(M*Llr)*(fdr+fdr_k1/2)+(Idm+Idm_k1/2)/T+(angle_get_wm/*-np*wr*/)*(Iqm+Iqm_k1/2));//(-Lr/Llr*Idm+ids+fdr/Llr+angle_get_wm/*np*wr*/*M/Rfe*Iqm)/M*Rfe;
		//float Iqm_k2=T/(1/T+Rfe/Lls+Rfe/Llr+Rfe/M)*(Rfe/(Lls*M)*(fqs+fqs_k1/2)+Rfe/(M*Llr)*(fqr+fqr_k1/2)+(Iqm+Iqm_k1/2)/T-(angle_get_wm/*-np*wr*/)*(Idm+Idm_k1/2));//(-Lr/Llr*Iqm+iqs-angle_get_wm/*np*wr*/*M/Rfe*Idm)/M*Rfe;

		//_______________
		
		
		float fds_k1=2.0*T*(Vds_T_1-fds*Rs/Lls+/*np*wr*/angle_get_wm_T_1*fqs+Rs*M/Lls*Idm);
		float fqs_k1=2.0*T*(Vqs_T_1-fqs*Rs/Lls-/*np*wr*/angle_get_wm_T_1*fds+Rs*M/Lls*Iqm);
		float fdr_k1=2.0*T*(-fdr*Rr/Llr+(angle_get_wm_T_1-np*wr_T_1)*fqr+M*Rr/Llr*Idm);
		float fqr_k1=2.0*T*(-fqr*Rr/Llr-(angle_get_wm_T_1-np*wr_T_1)*fdr+M*Rr/Llr*Iqm);
		float Idm_k1=2.0*T*(Rfe/(Lls*M)*fds-Idm*Rfe*(1.0/Lls+1.0/Llr+1.0/M)+Rfe/(M*Llr)*fdr+(angle_get_wm_T_1/*-np*wr*/)*Iqm);//(-Lr/Llr*Idm+ids+fdr/Llr+angle_get_wm/*np*wr*/*M/Rfe*Iqm)/M*Rfe;
		float Iqm_k1=2.0*T*(Rfe/(Lls*M)*fqs-Iqm*Rfe*(1.0/Lls+1.0/Llr+1.0/M)+Rfe/(M*Llr)*fqr-(angle_get_wm_T_1/*-np*wr*/)*Idm);//(-Lr/Llr*Iqm+iqs-angle_get_wm/*np*wr*/*M/Rfe*Idm)/M*Rfe;
		
		float fds_k2=2.0*T*((Vds_T)-(fds+fds_k1/2)*Rs/Lls+/*np*wr*/(angle_get_wm_T)*(fqs+fqs_k1/2)+Rs*M/Lls*(Idm+Idm_k1/2));
		float fqs_k2=2.0*T*((Vqs_T)-(fqs+fqs_k1/2)*Rs/Lls-/*np*wr*/(angle_get_wm_T)*(fds+fds_k1/2)+Rs*M/Lls*(Iqm+Iqm_k1/2));
		float fdr_k2=2.0*T*(-(fdr+fdr_k1/2)*Rr/Llr+((angle_get_wm_T)-(np*wr_T))*(fqr+fqr_k1/2)+M*Rr/Llr*(Idm+Idm_k1/2));
		float fqr_k2=2.0*T*(-(fqr+fqr_k1/2)*Rr/Llr-((angle_get_wm_T)-(np*wr_T))*(fdr+fdr_k1/2)+M*Rr/Llr*(Iqm+Iqm_k1/2));
		float Idm_k2=2.0*T*(Rfe/(Lls*M)*(fds+fds_k1/2)-(Idm+Idm_k1/2)*Rfe*(1.0/Lls+1.0/Llr+1.0/M)+Rfe/(M*Llr)*(fdr+fdr_k1/2)+((angle_get_wm_T)/*-np*wr*/)*(Iqm+Iqm_k1/2));//(-Lr/Llr*Idm+ids+fdr/Llr+angle_get_wm/*np*wr*/*M/Rfe*Iqm)/M*Rfe;
		float Iqm_k2=2.0*T*(Rfe/(Lls*M)*(fqs+fqs_k1/2)-(Iqm+Iqm_k1/2)*Rfe*(1.0/Lls+1.0/Llr+1.0/M)+Rfe/(M*Llr)*(fqr+fqr_k1/2)-((angle_get_wm_T)/*-np*wr*/)*(Idm+Idm_k1/2));//(-Lr/Llr*Iqm+iqs-angle_get_wm/*np*wr*/*M/Rfe*Idm)/M*Rfe;
		
		float fds_k3=2.0*T*((Vds_T)-(fds+fds_k2/2)*Rs/Lls+/*np*wr*/(angle_get_wm_T)*(fqs+fqs_k2/2)+Rs*M/Lls*(Idm+Idm_k2/2));
		float fqs_k3=2.0*T*((Vqs_T)-(fqs+fqs_k2/2)*Rs/Lls-/*np*wr*/(angle_get_wm_T)*(fds+fds_k2/2)+Rs*M/Lls*(Iqm+Iqm_k2/2));
		float fdr_k3=2.0*T*(-(fdr+fdr_k2/2)*Rr/Llr+((angle_get_wm_T)-(np*wr_T))*(fqr+fqr_k2/2)+M*Rr/Llr*(Idm+Idm_k2/2));
		float fqr_k3=2.0*T*(-(fqr+fqr_k2/2)*Rr/Llr-((angle_get_wm_T)-(np*wr_T))*(fdr+fdr_k2/2)+M*Rr/Llr*(Iqm+Iqm_k2/2));
		float Idm_k3=2.0*T*(Rfe/(Lls*M)*(fds+fds_k2/2)-(Idm+Idm_k2/2)*Rfe*(1.0/Lls+1.0/Llr+1.0/M)+Rfe/(M*Llr)*(fdr+fdr_k2/2)+((angle_get_wm_T)/*-np*wr*/)*(Iqm+Iqm_k2/2));//(-Lr/Llr*Idm+ids+fdr/Llr+angle_get_wm/*np*wr*/*M/Rfe*Iqm)/M*Rfe;
		float Iqm_k3=2.0*T*(Rfe/(Lls*M)*(fqs+fqs_k2/2)-(Iqm+Iqm_k2/2)*Rfe*(1.0/Lls+1.0/Llr+1.0/M)+Rfe/(M*Llr)*(fqr+fqr_k2/2)-((angle_get_wm_T)/*-np*wr*/)*(Idm+Idm_k2/2));		
		
		float fds_k4=2.0*T*((Vds/*+T*/)-(fds+fds_k3)*Rs/Lls+/*np*wr*/(angle_get_wm/*+T*/)*(fqs+fqs_k3)+Rs*M/Lls*(Idm+Idm_k3));
		float fqs_k4=2.0*T*((Vqs/*+T*/)-(fqs+fqs_k3)*Rs/Lls-/*np*wr*/(angle_get_wm/*+T*/)*(fds+fds_k3)+Rs*M/Lls*(Iqm+Iqm_k3));
		float fdr_k4=2.0*T*(-(fdr+fdr_k3)*Rr/Llr+((angle_get_wm/*+T*/)-(np*wr/*+T*/))*(fqr+fqr_k3)+M*Rr/Llr*(Idm+Idm_k3));
		float fqr_k4=2.0*T*(-(fqr+fqr_k3)*Rr/Llr-((angle_get_wm/*+T*/)-(np*wr/*+T*/))*(fdr+fdr_k3)+M*Rr/Llr*(Iqm+Iqm_k3));
		float Idm_k4=2.0*T*(Rfe/(Lls*M)*(fds+fds_k3)-(Idm+Idm_k3)*Rfe*(1.0/Lls+1.0/Llr+1.0/M)+Rfe/(M*Llr)*(fdr+fdr_k3)+((angle_get_wm/*+T*/)/*-np*wr*/)*(Iqm+Iqm_k3));//(-Lr/Llr*Idm+ids+fdr/Llr+angle_get_wm/*np*wr*/*M/Rfe*Iqm)/M*Rfe;
		float Iqm_k4=2.0*T*(Rfe/(Lls*M)*(fqs+fqs_k3)-(Iqm+Iqm_k3)*Rfe*(1.0/Lls+1.0/Llr+1.0/M)+Rfe/(M*Llr)*(fqr+fqr_k3)-((angle_get_wm/*+T*/)/*-np*wr*/)*(Idm+Idm_k3));		
		
		
		float fds_1=fds+1.0/6.0*(fds_k1+2.0*fds_k2+2.0*fds_k3+fds_k4);
		float fqs_1=fqs+1.0/6.0*(fqs_k1+2.0*fqs_k2+2.0*fqs_k3+fqs_k4);
		float fdr_1=fdr+1.0/6.0*(fdr_k1+2.0*fdr_k2+2.0*fdr_k3+fdr_k4);
		float fqr_1=fqr+1.0/6.0*(fqr_k1+2.0*fqr_k2+2.0*fqr_k3+fqr_k4);
		float Idm_1=Idm+1.0/6.0*(Idm_k1+2.0*Idm_k2+2.0*Idm_k3+Idm_k4);
		float Iqm_1=Iqm+1.0/6.0*(Iqm_k1+2.0*Iqm_k2+2.0*Iqm_k3+Iqm_k4);
...
float motor::torque_g(float Vds,float Vqs,float angle_get_wm){//torque generated, frame generic . used #######"backward euler method"######
...
float motor::get_torq_g(){return torq_g;};
float motor::get_m_Tr_calc_gl(){return m_Tr_calc_gl;};

void motor::set_theta_r(float t){theta_r=t;};
...
motor::~motor(){};
float motor::torque(float vas,float vbs,float vcs){
		
		fTorque<<"RFA: "<<RotorFluxAngle<<"vas, vbs, vcs "<< vas<<" "<<vbs<<" "<<vcs<<" "<<endl;

		float tt=  torque_g(ClarkePark(RotorFluxAngle/*np*theta_r*/,tThreePhase(vas,vbs,vcs)).d/*alpha*/,ClarkePark(RotorFluxAngle/*np*theta_r*/,tThreePhase(vas,vbs,vcs)).q/*beta*/,angle.get_wm());
				   //torque_g(ClarkePark(RotorFluxAngle/*np*theta_r*/,tThreePhase(vas,vbs,vcs)).d/*alpha*/,ClarkePark(RotorFluxAngle/*np*theta_r*/,tThreePhase(vas,vbs,vcs)).q/*beta*/,FOC::angle.get_wm());	
...
float motor::get_wr(float va,float vb,float vc/*float torq_g,float torq_L*/){
...
//-----------------	
	...
	void motor::set_torq_L(float torque){torq_L=torque;};//TODO remove in real
	
	//void motor::set_Rs(float rr){Rs=rr;};
	float motor::get_torq_L(){return torq_L;};//TODO remove in real
	
	float motor::get_wr(){return wr;};//TODO implement in REAL

	//float motor::get_Rs(){return Rs;};




	float control_loops::get_rot_fl_an(){return RotorFluxAngle;};

	control_loops::control_loops():/*v_(0.0,0.0),*//*IDQq_p(0.0),stc_p(0.0),*/ abc_voltage_svpwm(0.0,0.0,0.0),VDQ_rfa(0.0,0.0),/*sobre_Tr_comp(0.0),*/wmr__1(0.0),imrref(0.0),abc_voltage(0.0,0.0,0.0),IDQ(0.0,0.0)/*,VDQ(0.0,0.0)*/,/*VDQ_ant(0.0,0.0),*//*rot_fl_an(0.0),*/Theta_r(0.0),w_ref(0.0),V(0.0,0.0),IDQ_rotor(0.0,0.0),VDQ_rotor(0.0,0.0),IDQ_d_1(0.0),IDQ_d_(0.0),IDQ_d1(0.0),IDQ_d_p(0.0),IDQ_d_pp(0.0),IDQ_q_1(0.0),IDQ_q_(0.0),IDQ_q1(0.0),IDQ_q_p(0.0),IDQ_q_pp(0.0),wmr_(0.0),wmr1(0.0),wmr_p(0.0){
			...
			current_control_y_Min_pid_res=-6.45;//(-(2/3)*300);//100?
			current_control_y_Max_pid_res=6.45;//((2/3)*300);// 400//100?
	...
	void control_loops::set_w_ref(float wreff) {w_ref=wreff;};
	//void control_loops::set_imrref(float imrreff){imrref=imrreff;};
	float control_loops::get_w_ref(){return w_ref;};
...
////---------------------------	
		...
		//********** used in simulation to calc cos_phi
			...
			if ( iaa_p >0 && iaa_p*abc_current.a < 0 ) {/*if (desc_p==0.0){desc_p=n*T;} else {*/two_phi=n*T-desc_p;desc_p=n*T;cos_phi=n*T-cos_phi;float cos_phi_a=cos_phi/two_phi*2*PI;
		...
		//**********
			...
			RotorFluxAngle=angle.RotFluxAng_(IDQ.q,IDQ.d,T,m_Tr_calc_gl,(this->get_wr()*np));			
...
//PIDs------------------------------------------- To get Voltage direct and quadracture before decoupling: 
			...
			vel.set_acceleration_limit(vel_cel=0.03/*ACCEL*1000/3600*T_G_R/R*T*/);

	/*here*/float Wm = angle.get_wm();//Wm == we, utilizado nos papers, vel sincrona. aten.- electric speed- ..*number pole pairs	
			float Rd_we=Rs+Wm*Wm*M*M/Rm;
			float Rq_we=Rs+/*Rrr */1.0/(m_Tr_calc_gl*Lr)*M*M/*/Lr/Lr*/+Wm/*np*/*Wm/*np*/*M*M*Llr*Llr/Rm/Lr/Lr;//TODO ?substituir online Rs pelo calculado na constante de tempo d rotor 
			float IDQ_d_lma=0.0;
			...
			//if (IDQ.q>iqmax)current_control_y.set_/*???set point not process point*/process_point(iqmax);
			//else
...
//--------------Rotor Time Constant value adjust:			
			...
			fIDQ<<FIXED_FLOAT(n*T)<<" sec.;"<<" VDQ_rfa.d: "<<VDQ_rfa.d<<endl<<" IDQd: "<<IDQ.d<<" IDQq: "<<IDQ.q<<endl/*<<"(IDQd^2+IDQq^2)^(1/2): "<<sqrt(IDQd*IDQd+IDQq*IDQq)*//*IDQd+IDQq*/<<"IDC:"<<IDC/*<<"IDC2_3:"<<IDC2_3*/<<endl;
			fTorque<<FIXED_FLOAT(n*T)<<" sec.;"<<" VDQ_rfa.d: "<<VDQ_rfa.d<<endl<<" IDQd: "<<IDQ.d<<" IDQq: "<<IDQ.q<<endl/*<<"(IDQd^2+IDQq^2)^(1/2): "<<sqrt(IDQd*IDQd+IDQq*IDQq)*//*IDQd+IDQq*/<<"IDC:"<<IDC/*<<"IDC2_3:"<<IDC2_3*/<<endl;
						
			
			//cout<<"VDQ.q:"<<VDQ.q<<endl;
			fIDQ<<"VDQ.d :"<<VDQ.d<<endl<<"VDQ.q: "<<VDQ.q<<endl;
			fTorque<<"VDQ.d :"<<VDQ.d<<endl<<"VDQ.q: "<<VDQ.q<<endl;
			...
			if (n_rtc>50000 /*&&the following is because it dont work at 0 power: (IDQq < -100 || IDQq > 100 )*/) {//steady state TODO				n
				...
				float error=this->VDQ_rfa.d-(Rs*IDQ.d-angle.get_wm()*((Ls*Lr-M*M)/Lr)*IDQ.q);
				//s/*+*/=error;//dont work:integral controller, work proporcional only but need tune de following numeric value 
				//float pi_contr=(s)*/*the following value is the gain, adjusted from the rate of change of Rr(Tr=Lr/Rr)*/0.000004/*0.000025*/;
				if ((IDQ.q<-10.5*3/*10:maybe more secure 11.5*/ && wr<-/*30*/2 ) || (IDQ.q>10.5*3 && wr >2) /*10//-44//-40*//*&& VDQ.q>0*/ /*&& (IDQq <= -100 || IDQq >= 100 )*/){
				////if (torque_control.get_pid_result()<0){
				  sinal_=/*-*/1;}
				  else if((IDQ.q>10.5*3 && wr<-2) || (IDQ.q<-10.5*3 && wr >2 )){sinal_=-1;} 
				...
				fIDQ<<" TR of motor: "<<Lr/Rr<<" Tr estimated: "<<m_Tr_calc_gl/*Tr___*/<<" error: "<<error<<endl;
				}; 
	...
	//+Vdecouple-------------To get Voltage direct and quadracture after decoupling	
			...
			VDQ.q+=const_VDQ_q;
//-------------			
			fTorque<<"VDQ.d +decoupling:"<<VDQ.d<<endl;
			fIDQ<<"VDQ.d +decoupling: "<<VDQ.d<<endl;
			fTorque<<"VDQ.q +decoupling:"<<VDQ.q<<endl;
...
fImr<<FIXED_FLOAT(n*T)<<"sec.; imr (rotor magnetization current): "<<angle.get_imr()/*<<"TTTTTTTTTTTEEEEEEE"<<(3/2*np*M*M/Lr*IDQq*IDQd)*/<<endl;	
		...
		fTorque<<"get v alpha beta "<<V.alpha<<" "<<V.beta<<"rfa"<<RotorFluxAngle<<endl;
